...
def load_darknet_weights_in_yolo(yolo_model, darknet_weights_path):
    ...
    # Sort them by order of appearance. The first apparition does not have an
    # index, hence the [[0]] trick to avoid an error
    ...
    # Keep an index of which batch norm should be considered. If batch norm
    # is used with a convolution (meaning conv has no bias), the index is
    # incremented Otherwise (conv has a bias), index is kept still.
    ...
    for layer in conv_layers:
        ...
        if use_bias:
            ...
            # Read batch norm Reorder from darknet (beta, gamma, mean,
            # var) to TF (gamma, beta, mean, var)
            batch_norm_weights = np.fromfile(
                darknet_weight_file, dtype=np.float32, count=4 * filters
            ).reshape((4, filters))[[1, 0, 2, 3]]

        # Read kernel weights Reorder from darknet (filters, input_filters,
        # kernel_size[0], kernel_size[1]) to TF (kernel_size[0], kernel_size[
        # 1], input_filters, filters)
        ...
        conv_weights = (
            np.fromfile(darknet_weight_file, dtype=np.float32, count=conv_size)
                .reshape(
                (filters, input_filters, kernel_size[0], kernel_size[1]))
                .transpose([2, 3, 1, 0])
        )
...
def get_weights_by_keyword_or_path(weights, model):
    ...
    if (
        ...
        raise ValueError(
            "`weights` argument should either be in "
            "{AVAILABLE_PRETRAINED_WEIGHTS_OPTIONS}, "
...
def get_weights_by_keyword(weights, model):
    """
    Access pretrained weights based on keyword. Accepted keywords are:
    ["darknet"].
    Args:
        weights (str): Path or pretrained model keyword.
        model (tf.keras.Model): Model to load weights into.
...
def download_darknet_weights(yolov4_model):
    ...
    is_darknet_original_weights_available = \
        DARKNET_ORIGINAL_WEIGHTS_PATH.is_file()
...
def download_file_from_google_drive(file_id, destination, target_size=None):
    ...
    response = session.get(google_drive_base_url, params={"id": file_id},
                           stream=True)
    token = get_confirm_token(response)

    if token:
        params = {"id": file_id, "confirm": token}
        response = session.get(google_drive_base_url, params=params,
                               stream=True)
...
def save_response_content(response, destination, target_size=None):
    ...
    progbar = tf.keras.utils.Progbar(target_size) if target_size is not None \
        else None
