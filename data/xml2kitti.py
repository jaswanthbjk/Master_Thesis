...
def get_directory_xml_files(dir):
    return [join(dir, f) for f in listdir(dir) if
            isfile(join(dir, f)) and splitext(f)[1].lower() == ".xml"]
...
def main():
    if not check_argv(sys.argv):
        print(
            "Wrong arguments. You should specify xml files or directory with xml files")
